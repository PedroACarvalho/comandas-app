try:
    ...
    raise ImportError(
        "Required packages for database are not installed. Please install flask_sqlalchemy and flask_migrate."
    ) from e
...
def _inserir_itens_exemplo():
    ...
    if not Item.query.first():
        itens_exemplo = [
            Item(
                nome="X-Burger", descricao="Hambúrguer com queijo e salada", preco=15.90
            ),
            Item(
                nome="X-Salada",
                descricao="Hambúrguer com queijo, salada e tomate",
                preco=17.90,
            ),
