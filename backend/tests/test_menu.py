...
def test_criar_listar_atualizar_remover_item(client):
    ...
    item_id = (
        resp.json["item"]["item_id"]
        if "item" in resp.json and "item_id" in resp.json["item"]
        else resp.json["item"].get("id", None)
    )
    # Lista itens
    ...
    # Atualiza item
    resp3 = client.put(
        f"/api/itens/{item_id}", json={"nome": "Coca Zero", "preco": 6.0}
    )
