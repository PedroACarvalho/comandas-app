...
def criar_cliente_pedido_pago(client):
    ...
    cliente_resp = client.post(
        "/api/cliente", json={"nome": "ClientePagto", "mesa": 4001}
    )
...
def test_criar_pagamento_dinheiro(client):
    ...
    resp = client.post(
        "/api/pagamentos",
        json={"pedido_id": pedido_id, "metodo": "Dinheiro", "valor": 30.0},
    )
...
def test_criar_pagamento_pedido_inexistente(client):
    resp = client.post(
        "/api/pagamentos",
        json={"pedido_id": 99999, "metodo": "Dinheiro", "valor": 10.0},
    )
...
def test_criar_pagamento_pedido_nao_fechado(client):
    ...
    cliente_resp = client.post(
        "/api/cliente", json={"nome": "ClientePagto2", "mesa": 4002}
    )
    cliente_id = cliente_resp.json["cliente"]["cliente_id"]
    ...
    resp = client.post(
        "/api/pagamentos",
        json={"pedido_id": pedido_id, "metodo": "Dinheiro", "valor": 5.0},
    )
    assert resp.status_code == 400
    assert "error" in resp.json
